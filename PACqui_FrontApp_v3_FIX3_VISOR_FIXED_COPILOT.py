...
# --- Copilot Studio Agent Import (managed solution zip) ----------------------
# Este bloque permite "importar" un agente exportado de Copilot Studio (solución gestionada)
# y usar sus instrucciones/persona dentro del chat de PACqui.
#
# Importante:
# - El .zip NO se puede "ejecutar" en PACqui: es un paquete Dataverse/Power Platform.
# - Lo que sí podemos hacer es leer la configuración (persona, starters y SharePoint site)
#   y convertirla en un perfil de asistente para PACqui.

import zipfile

class AgentProfile:
    def __init__(self, name: str, instructions: str = "", response_instructions: str = "",
                 starters=None, sharepoint_sites=None):
        self.name = (name or "").strip() or "Agente"
        self.instructions = (instructions or "").strip()
        self.response_instructions = (response_instructions or "").strip()
        self.starters = starters or []  # [{"title":..., "text":...}, ...]
        self.sharepoint_sites = sharepoint_sites or []  # ["https://.../General", ...]

    def first_greeting(self) -> str:
        # Devuelve el starter "Bienvenida" si existe, si no el primero.
        for s in self.starters:
            if (s.get("title") or "").strip().lower() in ("bienvenida", "greeting", "hola"):
                t = (s.get("text") or "").strip()
                if t:
                    return t
        if self.starters:
            t = (self.starters[0].get("text") or "").strip()
            if t:
                return t
        return ""

def _parse_simple_yaml_block(raw: str, key: str) -> str:
    """Parser muy simple para bloques estilo YAML:
       key: |
         línea
         línea
       Devuelve el bloque sin la indentación."""
    ...
    for i, ln in enumerate(lines):
        if not in_block:
            if ln.strip() == f"{key}: |":
                in_block = True
            continue
        # terminó si aparece una nueva clave al nivel 0 (no indentada) tipo "algo:"
        if ln and (not ln.startswith(" ")) and re.match(r"^[A-Za-z0-9_]+\s*:", ln):
            break
        # quitar 2 espacios típicos de indentación si están
        out.append(ln[2:] if ln.startswith("  ") else ln)
    return "\n".join(out).rstrip()

def _parse_simple_yaml_value(raw: str, key: str) -> str:
    m = re.search(rf"^{re.escape(key)}:\s*(.+?)\s*$", raw or "", flags=re.MULTILINE)
    if not m:
        return ""
...
def _parse_conversation_starters(raw: str) -> list:
    lines = (raw or "").splitlines()
    starters = []
    i = 0
    # busca "conversationStarters:"
    while i < len(lines) and lines[i].strip() != "conversationStarters:":
        i += 1
    if i >= len(lines):
        return starters
    i += 1
    ...
    while i < len(lines):
        ln = lines[i]
        # si aparece nueva clave al nivel 0, termina
        if ln and (not ln.startswith(" ")) and re.match(r"^[A-Za-z0-9_]+\s*:", ln):
            break
        s = ln.strip()
        if s.startswith("- title:"):
            if cur:
                starters.append(cur)
            cur = {"title": s.split(":", 1)[1].strip().strip('"')}
        elif s.startswith("text:") and cur is not None:
            txt = s.split(":", 1)[1].strip()
            # puede venir entre comillas dobles
...
def load_copilot_agent_profile_from_zip(zip_path: str):
    """Lee un export de Copilot Studio (solution zip) y devuelve un AgentProfile.
       Si no encuentra la pieza GPT default, devuelve None."""
    try:
        zp = str(zip_path)
        ...
        with zipfile.ZipFile(zp, "r") as zf:
            ...
            # 1) localizar el data del componente GPT default (contiene instrucciones)
            gpt_data_name = None
            for n in names:
                if n.endswith("/data") and ".gpt.default" in n:
                    gpt_data_name = n
                    break
            # fallback: buscar por contenido
            if not gpt_data_name:
                for n in names:
                    if n.endswith("/data") and "botcomponents/" in n:
                        try:
                            raw = zf.read(n).decode("utf-8", errors="ignore")
                        except Exception:
                            continue
                        if "kind: GptComponentMetadata" in raw:
                            gpt_data_name = n
                            break
            if not gpt_data_name:
                return None

            gpt_raw = zf.read(gpt_data_name).decode("utf-8", errors="ignore")

            display = _parse_simple_yaml_value(gpt_raw, "displayName") or _parse_simple_yaml_value(gpt_raw, "name")
            instructions = _parse_simple_yaml_block(gpt_raw, "instructions")
            resp_instr = _parse_simple_yaml_block(gpt_raw, "responseInstructions")
            starters = _parse_conversation_starters(gpt_raw)

            # 2) extraer sites SharePoint configurados como knowledge sources (si existen)
            sp_sites = []
            for n in names:
                if n.endswith("/data") and "knowledge.SharePointSearchSource" in n:
                    try:
                        raw = zf.read(n).decode("utf-8", errors="ignore")
                    except Exception:
                        continue
                    m = re.search(r"\bsite:\s*(\S+)", raw)
                    if m:
                        sp_sites.append(m.group(1).strip())
            # Limpia duplicados manteniendo orden
            seen = set()
            sp_sites = [s for s in sp_sites if not (s in seen or seen.add(s))]

            return AgentProfile(
                name=display or "Agente Copilot",
                instructions=instructions,
                response_instructions=resp_instr,
                starters=starters,
                sharepoint_sites=sp_sites
            )
    except Exception:
        return None
# ---------------------------------------------------------------------------
...
class AdminPanel(ttk.Notebook):
    ...
    def _danger_confirm(self, title: str, msg: str, must_type: str  = None) -> bool:
...
class ChatWithLLM(ChatFrame):
    def __init__(self, master, data: DataAccess, llm: LLMService, app=None):
        ...
        self._agent_profiles = {"PACqui": AgentProfile("PACqui")}
        self._active_agent = "PACqui"
        self._copilot_import_hint = ""  # info breve (SharePoint site, etc.)
        self._maybe_auto_load_copilot_agent()
    ...
    # --- AGENTES (perfiles Copilot/PACqui) ---------------------------------
    def _get_active_profile(self):
        try:
            p = self._agent_profiles.get(getattr(self, "_active_agent", "PACqui"))
            if p:
                return p
            return self._agent_profiles.get("PACqui") or AgentProfile("PACqui")
        except Exception:
            return AgentProfile("PACqui")

    def _assistant_label(self) -> str:
        # etiqueta que aparece en el chat (columna izquierda)
        p = self._get_active_profile()
        return (p.name or "PACqui").strip() or "PACqui"

    def _on_agent_changed(self):
        try:
            name = (self.var_agent.get() or "PACqui").strip()
        except Exception:
            name = "PACqui"
        if name not in getattr(self, "_agent_profiles", {}):
            name = "PACqui"
        self._active_agent = name
        # info breve para el usuario
        try:
            p = self._get_active_profile()
            if p.sharepoint_sites:
                self._copilot_import_hint = f"SharePoint: {p.sharepoint_sites[0]}"
            else:
                self._copilot_import_hint = ""
        except Exception:
            self._copilot_import_hint = ""

    def _refresh_agent_combo(self, select=None):
        try:
            if not hasattr(self, "cbo_agent"):
                return
            vals = list(getattr(self, "_agent_profiles", {}).keys())
            self.cbo_agent.configure(values=vals)
            if select and select in vals:
                self.var_agent.set(select)
                self._active_agent = select
        except Exception:
            pass

    def _maybe_auto_load_copilot_agent(self):
        """Carga opcional de un zip de Copilot Studio si el usuario lo configura vía ENV.
           - PACQUI_COPILOT_ZIP: ruta al .zip exportado
           - PACQUI_COPILOT_AUTO=1: activa ese agente por defecto"""
        try:
            import os as _os
            zp = (_os.getenv("PACQUI_COPILOT_ZIP", "") or "").strip()
            if not zp or not os.path.exists(zp):
                return
            prof = load_copilot_agent_profile_from_zip(zp)
            if not prof:
                return
            name = prof.name
            # evita colisiones
            if name in self._agent_profiles:
                i = 2
                base = name
                while f"{base} ({i})" in self._agent_profiles:
                    i += 1
                name = f"{base} ({i})"
                prof.name = name
            self._agent_profiles[name] = prof
            self._refresh_agent_combo()
            if (_os.getenv("PACQUI_COPILOT_AUTO", "0") or "").strip() == "1":
                self._active_agent = name
                try:
                    if hasattr(self, "var_agent"):
                        self.var_agent.set(name)
                except Exception:
                    pass
        except Exception:
            pass

    def _import_copilot_agent_dialog(self):
        from tkinter import messagebox, filedialog
        try:
            zp = filedialog.askopenfilename(
                title="Selecciona el ZIP exportado de Copilot Studio",
                filetypes=[("ZIP (Copilot export)", "*.zip"), ("Todos", "*.*")]
            )
        except Exception:
            zp = ""
        if not zp:
        ...
        if not prof:
            try:
                messagebox.showerror(APP_NAME, "No he encontrado la configuración GPT del agente dentro del ZIP.")
            except Exception:
                pass
            return
        ...
        self._agent_profiles[name] = prof
        self._refresh_agent_combo(select=name)
        self._on_agent_changed()

        # Mensaje visible en chat para confirmar
        hint = ""
        try:
            if prof.sharepoint_sites:
                hint = f"\nFuente principal: {prof.sharepoint_sites[0]}"
        except Exception:
            pass
        try:
            messagebox.showinfo(APP_NAME, f"Agente importado: {name}{hint}")
        except Exception:
            pass

    def _system_policy_rag(self) -> str:
        """Política SYSTEM para el modo normal (FRAGMENTOS/ÍNDICE/CONCEPTOS)."""
        # default PACqui (comportamiento actual)
        if getattr(self, "_active_agent", "PACqui") == "PACqui":
            return (
                "Eres PACqui, asistente experto en PAC/PEPAC/SICOP. Responde SIEMPRE en español neutro. "
                "Jerarquía de fuentes:\n"
                "1) Si hay [FRAGMENTOS], usa EXCLUSIVAMENTE esos fragmentos como base factual.\n   Las citas [n] SOLO pueden aparecer en un bloque final 'Fuentes:' (nunca incrustadas en frases).\n   Si no puedes listar 'Fuentes:' con rutas reales, NO uses [n].\n"
        ...
        # Perfil Copilot importado
        p = self._get_active_profile()
        sp_hint = ""
        try:
            if p.sharepoint_sites:
                sp_hint = f"\nFuente principal sugerida (SharePoint): {p.sharepoint_sites[0]}"
        except Exception:
            sp_hint = ""

        instr = (p.instructions or "").strip()
        resp = (p.response_instructions or "").strip()
        ...
        return (
            ...
            + ("Jerarquía de fuentes (obligatoria):\n"
               "1) Si hay [FRAGMENTOS], úsalos EXCLUSIVAMENTE como base factual.\n   Las citas [n] SOLO pueden aparecer en un bloque final 'Fuentes:' (nunca incrustadas en frases).\n   Si no puedes listar 'Fuentes:' con rutas reales, NO uses [n].\n"
               "2) Si NO hay [FRAGMENTOS] pero hay [CONCEPTOS], responde anclado a esos conceptos (sin [n]) y di que vienen de [CONCEPTOS].\n"
               "3) Si no hay [FRAGMENTOS] ni [CONCEPTOS] y solo hay [ÍNDICE], limita la salida a sugerir rutas (sin inventar).\n"
               "4) Si no hay nada útil, di claramente 'no lo sé' y sugiere dónde mirar (rutas/SharePoint).\n"
               "Prohibido: inventar datos, citar fuentes inexistentes, o dar instrucciones fuera del ámbito del repositorio.\n")
            + sp_hint
        )

    def _system_policy_obs(self) -> str:
        """Política SYSTEM para el modo SOLO observaciones (sin RAG)."""
        if getattr(self, "_active_agent", "PACqui") == "PACqui":
            return (
                "Eres PACqui, asistente experto en PAC/PEPAC/SICOP. Responde SIEMPRE en español neutro. "
                "Usa EXCLUSIVAMENTE las OBSERVACIONES y RUTAS del índice para orientar. "
                "NO inventes contenido del documento. Si no hay observaciones útiles, di que no hay evidencia en el índice."
            )
        ...
        try:
            if p.sharepoint_sites:
                sp_hint = f" Fuente principal sugerida: {p.sharepoint_sites[0]}"
        except Exception:
            pass
        ...
        return (
            ...
            + (instr + " " if instr else "")
            + (resp + " " if resp else "")
            + "Modo: SOLO ÍNDICE. Usa únicamente OBSERVACIONES/RUTAS del índice; no inventes hechos."
            + sp_hint
        )
    ...
    def _build_ui(self):
        ...
        super()._build_ui()
        # Sustituye el Entry (una línea) por un Text multilínea
        ...
        # Caja de texto grande (ajusta height si la quieres más/menos alta)
        ...
        # Reencaminar ENTER y el botón "Enviar" al LLM (Shift+Enter = nueva línea)
        try:
            def _on_return(e):
        ...
        # --- Selector de agente (PACqui / Copilot importado) ---
        try:
            self.var_agent = tk.StringVar(value=getattr(self, "_active_agent", "PACqui"))
            self.cbo_agent = ttk.Combobox(parent, textvariable=self.var_agent, state="readonly", width=18)
            self.cbo_agent.configure(values=list(getattr(self, "_agent_profiles", {}).keys()))
            self.cbo_agent.pack(side="left", padx=(8, 0))
            self.cbo_agent.bind("<<ComboboxSelected>>", lambda e: self._on_agent_changed())
            ttk.Button(parent, text="Importar agente…", command=self._import_copilot_agent_dialog).pack(side="left", padx=(6, 0))
    ...
    def _persona_line_from_llm(self, query_text: str, titles: list[str], n: int) -> str:
        ...
        """
        self.after(0, lambda: self.progress("Stream no abrió en tiempo → llamo al modelo sin streaming."))
    ...
    def _compose_system_budgeted(self, user_text: str, max_tokens: int = 256):
        ...
        base_sys = self._system_policy_rag()
...
# Contexto inicial (auto-escala con el ctx del modelo)
    ...
    def _send_llm(self):
        ...
        # Guardamos el último turno "real" (evita que ACKs pisen el historial).
        _prev_real = (getattr(self, "_last_user_q", "") or "").strip()
        ...
        if is_first_turn and smalltalk:
            try:
                greet = (self._get_active_profile().first_greeting() or "").strip()
            except Exception:
                greet = ""
            if not greet:
                greet = "Hola 👋. Escribe tu pregunta y te sugeriré rutas relevantes."
            self._append_chat(self._assistant_label(), greet)
            self._user_turns += 1
            return
        ...
        if is_first_turn and _re_small.fullmatch(
            ...
            # Respuesta genérica si el perfil no define otra cosa
            self._append_chat(self._assistant_label(), "¡Todo bien! 🙂 ¿En qué te ayudo del repositorio?")
            self._user_turns += 1
            return

        # ACK / smalltalk (cualquier turno): no dispares RAG/LLM ni ancles contexto
        try:
            if self._is_ack_or_smalltalk(q):
                self._append_chat(self._assistant_label(),
                                 "Perfecto 🙂. Cuando quieras, dime tu pregunta y te sugiero rutas relevantes.")
                self._user_turns += 1
                return
        except Exception:
            pass

        # Actualiza el contexto conversacional SOLO para turnos "reales"
        self._prev_user_q = _prev_real
        self._last_user_q = q
        ...
        if (not self.llm.is_loaded()) or self.notes_only:
            ...
            try:
                ...
                self._append_chat(self._assistant_label(), "No hay modelo o está en modo 'Solo índice'.")
            return
        ...
        def worker():
            ...
            try:
                ...
                self.after(0, lambda m=_msg: self._append_chat(self._assistant_label(), m))
    ...
    def _is_ack_or_smalltalk(self, text: str) -> bool:
        """Detecta ACKs / smalltalk muy corto (bien/ok/vale/gracias...) para:
        - NO disparar RAG/LLM en turnos de asentimiento.
        - NO anclar estos turnos al anterior (evita respuestas desalineadas).

        Heurística conservadora:
        - Si contiene signos de interrogación (¿/?) se asume pregunta (NO es ACK).
        - Se limpia puntuación y se compara contra un set de ACKs.
        """
        import re
        raw = (text or "").strip()
        if not raw:
        ...
        # Si hay interrogación, probablemente es una pregunta, no un ACK
        if "?" in raw or "¿" in raw:
            return False
        ...
        # Variantes cortas tipo "bien gracias", "ok perfecto", etc.
        if len(s) <= 18:
            head = s.split(" ", 1)[0]
            if head in {"ok", "vale", "bien", "perfecto", "genial", "gracias"}:
                return True
    ...
    def _rewrite_query_for_retrieval(self, user_text: str) -> str:
        ...
        # ACK/smalltalk: NO anclar al turno anterior para recuperación
        if hasattr(self, '_is_ack_or_smalltalk') and self._is_ack_or_smalltalk(q):
            return q
    ...
    def _pick_best_hit(self, hits: list[dict], q: str) -> dict :
        import re
        ql = (q or "").lower()
    ...
    def _compose_persona_from_obs(self, user_text: str, max_tokens: int = 192):
        ...
        base_sys = self._system_policy_obs()
...
# 3) Ensamblamos contextos
    ...
    def _reply_with_observations(self, query_text: str):
        ...
        # 4) Volcar al chat
        self._append_chat(self._assistant_label(), msg)
    ...
    def _append_stream_text(self, txt: str, end_turn: bool = False):
        ...
        try:
            ...
            if not self._in_stream:
                ...
                self.txt_chat.insert("end", f"{self._assistant_label()}:\n", ("who",))
        ...
        def _worker(
            ...
            try:
                ...
                try:
                    ...
                    state["last_ts"] = time.time()  # medir primer token desde stream abierto
                except Exception as e:
                ...
                try:
                    ...
                    if state["tokens"] == 0:
                        ...
                        _p(f"Primer token recibido ✅ ({dt_first} ms)")
